...
faqItems:
  - q: "패트리어트 미사일 소프트웨어 실패의 원인은 무엇인가요?"
    a: "무기 제어 소프트웨어의 24비트 레지스터 제한으로 인해 1/10초 정수 클럭 틱을 실수로 변환할 때 정밀도 손실이 발생했습니다. 100시간 이상 연속 가동 후 0.3433초의 타이밍 오차가 누적되어 레이더 거리 게이트가 687미터 이동했습니다."
  - q: "왜 0.3433초의 오차가 치명적이었나요?"
    a: "스커드 미사일은 초당 약 1,676미터로 비행합니다. 0.3433초의 오차는 레이더 거리 게이트를 687미터 이동시켜 시스템이 엉뚱한 위치를 탐색하게 만들어 스커드 요격에 실패했습니다."
  - q: "공격 전에 버그에 대해 알고 있었나요?"
    a: "네. 패트리어트 프로젝트 사무소는 2월 11일 이스라엘 측으로부터 거리 게이트 저하 데이터를 받았습니다. 2월 16일 수정 소프트웨어가 출시되고 21일 경고가 발송되었으나 임계값이 명시되지 않았고 패치는 공격 하루 뒤 도착했습니다."
  - q: "이것은 하드웨어 결함이었나요, 소프트웨어 결함이었나요?"
    a: "아키텍처와 운영의 불일치였습니다. 24비트 레지스터는 유한한 정밀도 제약을 부과했고, 군 작전 배치가 원래 설계 가정을 초과하여 장기간 연속 가동을 수행하면서 누적되었습니다."
primary_sources:
...
스커드 미사일이 영공에 진입했을 때 패트리어트 레이더가 이를 감지했습니다. 그러나 소프트웨어의 24비트 제한 정밀도 시간 변환(limited-precision time conversion)의 유한한 정밀도로 인해 시스템의 내부 클럭이 약 0.3433초 어긋나 있었습니다. ([GAO](https://www.gao.gov/pdf/product/215614)) 스커드 미사일의 비행 속도는 초당 약 1,676미터이므로, 이 타이밍 오류는 패트리어트의 계산된 거리 게이트를 이동시켜 날아오는 미사일이 더 이상 추적 창 안의 중앙에 적절히 위치하지 않게 만들었습니다.
...
결과적으로 패트리어트가 계속 가동됨에 따라 추적 계산은 점점 더 부정확해졌습니다. GAO는 시간 부정확성이 1시간 후 약 0.0034초, 8시간 후 0.0275초, 그리고 약 100시간 후 0.3433초에 도달했다고 계산했습니다. ([GAO](https://www.gao.gov/assets/imtec-92-26.pdf))

### 4.5 추적의 결과 (The tracking consequence)
그러한 시간 부정확성은 예상된 레이더 거리 게이트의 변위로 이어졌습니다. 약 100시간 경과 시, GAO는 거리 게이트 변위가 약 687미터에 이른다고 계산했습니다. ([GAO](https://www.gao.gov/assets/imtec-92-26.pdf))
...
GAO는 시간 부정확성이 1시간 후 약 0.0034초, 8시간 후 0.0275초, 약 100시간 후 0.3433초에 도달했다고 계산했습니다. ([GAO](https://www.gao.gov/assets/imtec-92-26.pdf))
...
> **증거가 확립하지 않는 것:**
> - **정확한 테스트 스위트 커버리지:** `[UNKNOWN — INSUFFICIENT RECORD]` GAO는 패트리어트가 전술 탄도 미사일을 방어하는 데 이전에 사용된 적이 없었으며, 걸프전에서 겪었던 긴 연속 가동 시간에 대해서도 테스트되지 않았다고 보고했습니다. 이용 가능한 보고서는 원래 테스트 스위트의 완전한 재구성을 제공하지 않습니다.
> - **엔지니어의 의도:** `[UNKNOWN — INSUFFICIENT RECORD]` 이용 가능한 증거는 원래의 제한 정밀도 표현(limited-precision representation)에 대한 정확한 설계 근거나 소프트웨어가 개발될 때 만들어진 전체 런타임 가정 세트를 확립하지 않습니다.
...
소프트웨어 재사용과 검증되지 않은 런타임 가정이 시스템 안전 인터록을 우회한 유사한 아키텍처 결함은 [테락-25 방사선 과다조사 참사](/ko/blog/therac-25-radiation-overdose-race-condition)와 수치 변환 오버플로로 폭발한 [아리안 5호 501편 비행 사고](/ko/blog/ariane-5-flight-501-integer-overflow)에서도 반복되었습니다. 정량화되지 않은 운영자 경고에 의존하기보다 자동화된 어설션(assertion)으로 모든 운영 가정을 시스템 경계 내에 구속해야 합니다.
